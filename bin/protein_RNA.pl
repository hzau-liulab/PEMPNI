...
system "mkdir -p $workdir/$mut_name/energy_feature/GB2";
...
system "mkdir -p $workdir/$mut_name/bio_feature/JSD/pssm";
system "mkdir -p $workdir/$mut_name/bio_feature/JSD/cscores";
system "mkdir -p $workdir/$mut_name//bio_feature/ENDES";
...
open(OUTTTTT,">$workdir/$mut_name/data_process/WT_rna.pdb") or die "can not open $!\n";
open(OUTT,">$workdir/$mut_name/data_process/pre_rna.pdb") or die "can not open $!\n";
...
open(IN, "$workdir/$mut_name/data_process/pre_rna.pdb") or die "can not open $!\n";
my @nulnul=<IN>;
close IN;

open(OOUT,">$workdir/$mut_name/data_process/pree_rna.pdb") or die "can not open $!\n";
...
open(INN, "$workdir/$mut_name/data_process/pree_rna.pdb") or die "can not open $!\n";
my @preedna=<INN>;
close INN;
...
open(IN, "$workdir/$mut_name/data_process/WT_rna.pdb") or die "can not open $!\n";
my @nulll=<IN>;
close IN;
...
open(OUTTTTT,">$workdir/$mut_name/data_process/MT_rna.pdb") or die "can not open $!\n";
my @nul;
...
open(IN,"$workdir/$mut_name/data_process/MT_rna.pdb") or die "can not open $!\n";
while (my $line = <IN>) {
    chomp $line;
    print OU "$line\n";
}
...
push (@na,"WT_pro");
for my $na (@na){
open(IN,"$workdir/$mut_name/data_process/$na.pdb") or die "can not open this files\n";
my %hash;
while (my $line=<IN>) {
...
open(OUT,">$workdir/$mut_name/data_process/$na.fas") or die "can not open this files\n";
print OUT ">"."$na\n";
foreach my $i (@fasta){
    print OUT "$i"
}
...
my $nn=1;
open(OUTT,">$workdir/$mut_name/data_process/$na"."_seq.txt") or die "can not open this files\n";
for my $key (sort{$a<=>$b} keys %hash)
{
    my $value=&text($hash{$key});
...
open(OUT,">","$workdir/$mut_name/bio_feature/ASA/wt_bind_asa.txt") or die "$!";
open(IN,"<","$workdir/$mut_name/bio_feature/ASA/$pdb_chain.rsa") or die "$!";
while (my $line = <IN>) {
    my @line = split(/\s+/,$line);
...
open(OUT,">","$workdir/$mut_name/bio_feature/ASA/wt_unbind_asa.txt") or die "$!";
open(IN,"<","$workdir/$mut_name/bio_feature/ASA/WT_pro.rsa") or die "$!";
while (my $line = <IN>) {
    my @line = split(/\s+/,$line);
    if ($line[3]=~ /\d+/ && $line[0] =~/RES/ && exists $aa{$line[1]}) {
       print OUT "$line[3]\t$aa{$line[1]}\t$line[4]\t$line[6]\t$line[8]\t$line[10]\t$line[12]\n";
...
open(OUT,">","$workdir/$mut_name/bio_feature/ASA/mt_bind_asa.txt") or die "$!";
open(IN,"<","$workdir/$mut_name/bio_feature/ASA/MT_$pdb_chain.rsa") or die "$!";
while (my $line = <IN>) {
    my @line = split(/\s+/,$line);
    if ($line[3]=~ /\d+/ && $line[0] =~/RES/ && exists $aa{$line[1]}) {
       print OUT "$line[3]\t$aa{$line[1]}\t$line[4]\t$line[6]\t$line[8]\t$line[10]\t$line[12]\n";
...
open(OUT,">","$workdir/$mut_name/bio_feature/ASA/mt_unbind_asa.txt") or die "$!";
open(IN,"<","$workdir/$mut_name/bio_feature/ASA/MT_pro.rsa") or die "$!";
while (my $line = <IN>) {
    my @line = split(/\s+/,$line);
    if ($line[3]=~ /\d+/ && $line[0] =~/RES/ && exists $aa{$line[1]}) {
       print OUT "$line[3]\t$aa{$line[1]}\t$line[4]\t$line[6]\t$line[8]\t$line[10]\t$line[12]\n";
...
open(OUT,">","$workdir/$mut_name/bio_feature/ASA/wt_dasa.txt") or die "$!";
open(IN,"<","$workdir/$mut_name/bio_feature/ASA/wt_unbind_asa.txt") or die "$!";
chomp(my @wt_pro_asa= <IN>);
close IN;
open(INN,"<","$workdir/$mut_name/bio_feature/ASA/wt_bind_asa.txt") or die "$!";
...
open(OUT,">","$workdir/$mut_name/bio_feature/ASA/mt_dasa.txt") or die "$!";
open(IN,"<","$workdir/$mut_name/bio_feature/ASA/mt_unbind_asa.txt") or die "$!";
chomp(my @mt_pro_asa = <IN>);
close IN;
open(INN,"<","$workdir/$mut_name/bio_feature/ASA/mt_bind_asa.txt") or die "$!";
...
open(INN,"<","$workdir/$mut_name/bio_feature/ASA/mt_dasa.txt") or die "$!"; 
while (<INN>) {
        chomp;
        $_ =~ s/^\s+//;
        my @tem = split(/\s+/);
...
open(INN,"<","$workdir/$mut_name/bio_feature/ASA/wt_dasa.txt") or die "$!";   
while (<INN>) {
        chomp;
        $_ =~ s/^\s+//;
        my @tem = split(/\s+/);
...
open(OUT,">","$workdir/$mut_name/bio_feature/ASA/IR-dASA.txt") or die "$!";
...
open(OUT,">","$workdir/$mut_name/bio_feature/ASA/bRSA.txt") or die "$!";
...
open(OUT,">","$workdir/$mut_name/bio_feature/ASA/uRSA.txt") or die "$!";
...
system"$software_dir/ncbi-blast-2.11.0+/bin/psiblast -query $workdir/$mut_name/data_process/WT_pro.fas -db $software_dir/nr/nr  -num_iterations 3  -out 1 -out_ascii_pssm $workdir/$mut_name/bio_feature/JSD/pssm/WT.pssm";
system"$software_dir/ncbi-blast-2.11.0+/bin/psiblast -query $workdir/$mut_name/data_process/MT_pro.fas -db $software_dir/nr/nr  -num_iterations 3  -out 1 -out_ascii_pssm $workdir/$mut_name/bio_feature/JSD/pssm/MT.pssm";
system"/usr/bin/perl $workdir/scripts/4_cscore+_zscore+logical_gzz.pl $workdir";

my %wt_has;
open(IN, "$workdir/$mut_name/data_process/WT_pro_seq.txt") or die "$!";
while (my $line = <IN>) {
    chomp ($line);
    my @line = split(/\s+/,$line);
...
my $wt_JSD;
open(IN, "$workdir/$mut_name/bio_feature/JSD/cscores/WT.txt") or die "$!";
while (my $line = <IN>) {
    chomp ($line);
...
my %mt_has;
open(IN, "$workdir/$mut_name/data_process/MT_pro_seq.txt") or die "$!";
while (my $line = <IN>) {
    chomp ($line);
    my @line = split(/\s+/,$line);
    $wt_has{$line[2]}=$line[0];
...
my $mt_JSD;
open(IN, "$workdir/$mut_name/bio_feature/JSD/cscores/MT.txt") or die "$!";
while (my $line = <IN>) {
    chomp ($line);
    if ($line =~/^$wt_has{$mutate_pos}/) {
        my @line = split(/\s+/,$line);
...
open(OUT, ">$workdir/$mut_name/bio_feature/JSD/JSD.txt") or die "$!";
my $cha = $mt_JSD - $wt_JSD;
print OUT "$cha\t$wt_JSD\n";
close OUT;

################################################################################Calculate ENDES features####################################################################################
system "cp $workdir/$mut_name/data_process/WT_pro.pdb $software_dir/enrich/unbound";
system "cp $workdir/$mut_name/data_process/MT_pro.pdb $software_dir/enrich/unbound";
chdir "$software_dir/enrich";
system "rm energy";
system "./processpdb WT_pro";
system "mv $software_dir/enrich/energy $workdir/$mut_name/bio_feature/ENDES/$pdb_chain'.'_enrich.txt";
system "rm energy";
system "./processpdb MT_pro";
system "mv $software_dir/enrich/energy $workdir/$mut_name/bio_feature/ENDES/$mut_name'.'_enrich.txt";
system "rm energy";

system "cp $workdir/$mut_name/data_process/WT_pro.pdb $software_dir/pinup";
system "cp $workdir/$mut_name/data_process/MT_pro.pdb $software_dir/pinup";
chdir "$software_dir/pinup";
system "rm score";
system "rm p001.pdb";
system "mv WT_pro.pdb p001.pdb";
system "./pinup";
system "mv $software_dir/pinup/score $workdir/$mut_name/bio_feature/ENDES/$pdb_chain'.'_pinup.txt";
system "rm score";
system "rm p001.pdb";
system "mv MT_pro.pdb p001.pdb";
system "./pinup";
system "mv $software_dir/pinup/score $workdir/$mut_name/bio_feature/ENDES/$mut_name'.'_pinup.txt";
system "rm score";
system "rm p001.pdb";
...
open(AA,"<","$workdir/$mut_name/bio_feature/ENDES/$pdb_chain"."._pinup.txt") or die "$!";
    while (<AA>) {
        chomp;
        my @temp = split(/\s+/);        
...
open(AA,"<","$workdir/$mut_name/bio_feature/ENDES/$pdb_chain"."._enrich.txt") or die "$!"; 
while (<AA>) {
    chomp;
     my @temp = split(/\s+/);          
...
open(AA,"<","$workdir/$mut_name/bio_feature/ENDES/$mut_name"."._pinup.txt") or die "$!";
    while (<AA>) {
        chomp;
        my @temp = split(/\s+/);        
...
open(AA,"<","$workdir/$mut_name/bio_feature/ENDES/$mut_name"."._enrich.txt") or die "$!"; 
while (<AA>) {
    chomp;
    my @temp = split(/\s+/);        
...
open(OUT,">","$workdir/$mut_name/bio_feature/ENDES/ENDES.txt") or die "$!";
...
print OUT "\t";
open(INN,"$workdir/$mut_name/bio_feature/ASA/bRSA.txt") or die "$!";
while (my $line = <INN>) {
    chomp $line;
    my @line = split(/\s+/,$line);
...
open(INN,"$workdir/$mut_name/bio_feature/ENDES/ENDES.txt") or die "$!";
while (my $line = <INN>) {
    chomp $line;
    my @line = split(/\s+/,$line);
     push(@bio_feature,@line);
...
open(INN,"$workdir/$mut_name/bio_feature/ASA/IR-dASA.txt") or die "$!";
while (my $line = <INN>) {
    chomp $line;
    my @line = split(/\s+/,$line);
     push(@bio_feature,@line);
...
open(INN,"$workdir/$mut_name/bio_feature/JSD/JSD.txt") or die "$!";
while (my $line = <INN>) {
    chomp $line;
    my @line = split(/\s+/,$line);
     push(@bio_feature,@line);
...
open(INN,"$workdir/$mut_name/bio_feature/ASA/uRSA.txt") or die "$!";
while (my $line = <INN>) {
    chomp $line;
    my @line = split(/\s+/,$line);
     push(@bio_feature,@line);
...
system "mkdir -p $workdir/$mut_name/energy_feature/GB2/wt";
system "cp $workdir/$mut_name/data_process/WT_com.pdb $workdir/$mut_name/energy_feature/GB2/wt";
system "cp $workdir/$mut_name/data_process/WT_rna.pdb $workdir/$mut_name/energy_feature/GB2/wt";
system "cp $workdir/$mut_name/data_process/WT_pro.pdb $workdir/$mut_name/energy_feature/GB2/wt";
system "mv $workdir/$mut_name/energy_feature/GB2/wt/WT_com.pdb $workdir/$mut_name/energy_feature/GB2/wt/com.pdb";
system "mv $workdir/$mut_name/energy_feature/GB2/wt/WT_pro.pdb $workdir/$mut_name/energy_feature/GB2/wt/pro.pdb";
system "mv $workdir/$mut_name/energy_feature/GB2/wt/WT_rna.pdb $workdir/$mut_name/energy_feature/GB2/wt/dna.pdb";
system "cp $workdir/scripts/GB2/* $workdir/$mut_name/energy_feature/GB2/wt";
chdir "$workdir/$mut_name/energy_feature/GB2/wt";
system "sh $workdir/$mut_name/energy_feature/GB2/wt/amber.sh";
system "mv $workdir/$mut_name/energy_feature/GB2/wt/Final_result_mmpbsa_GB2_pair.dat $workdir/$mut_name/energy_feature/GB2/$pdb_chain'.'_pair.dat";

system "mkdir -p $workdir/$mut_name/energy_feature/GB2/mt";
system "cp $workdir/$mut_name/data_process/MT_com.pdb $workdir/$mut_name/energy_feature/GB2/mt";
system "cp $workdir/$mut_name/data_process/MT_rna.pdb $workdir/$mut_name/energy_feature/GB2/mt";
system "cp $workdir/$mut_name/data_process/MT_pro.pdb /$workdir/$mut_name/energy_feature/GB2/mt";
system "mv $workdir/$mut_name/energy_feature/GB2/mt/MT_com.pdb $workdir/$mut_name/energy_feature/GB2/mt/com.pdb";
system "mv $workdir/$mut_name/energy_feature/GB2/mt/MT_pro.pdb $workdir/$mut_name/energy_feature/GB2/mt/pro.pdb";
system "mv $workdir/$mut_name/energy_feature/GB2/mt/MT_rna.pdb $workdir/$mut_name/energy_feature/GB2/mt/dna.pdb";
system "cp $workdir/scripts/GB2/* $workdir/$mut_name/energy_feature/GB2/mt";
chdir "$workdir/$mut_name/energy_feature/GB2/mt";
system "sh $workdir/$mut_name/energy_feature/GB2/mt/amber.sh";
system "mv $workdir/$mut_name/energy_feature/GB2/mt/Final_result_mmpbsa_GB2_pair.dat $workdir/$mut_name/energy_feature/GB2/$mut_name'.'_pair.dat";

open(IN,"<","$workdir/$mut_name/energy_feature/GB2/$pdb_chain"."._pair.dat") or die "$!";
my @wt_data = <IN>;
close IN;
...
open(IN,"<","$workdir/$mut_name/energy_feature/GB2/$mut_name"."._pair.dat") or die "$!";
my @mt_data = <IN>;
close IN;
...
open(OUT,">","$workdir/$mut_name/energy_feature/GB2/EWC.txt") or die "$!";
foreach my $kk(0..$#wt) {
                my $cha = $mt[$kk] - $wt[$kk];
                print OUT $cha,"\t";
            }
...
system "/home/yaojiang/anaconda3/bin/python $workdir/scripts/MPR-regression.py $workdir $mut_name";

########################Random forest classification#############################
system "/home/yaojiang/anaconda3/bin/python $workdir/scripts/MPR-classification.py $workdir $mut_name";
