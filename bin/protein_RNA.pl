...
#Examples:perl protein_DNA.pl 1FEU A D87E
...
my $tem = $pdb_name."_".$chain_name."_".$mutate_res;   
my @te = split(/_/,$tem);                 
my $site = $mutate_pos;                                
my $wt = substr($mutate_res,0,1);         
my @pro_coor;                             
my %pdb_coor;                           
open(INN,"<","$workdir/output/data_process/WT_com.pdb") or die "$!";    
while (<INN>) {
    chomp;
    if ($_ =~ /^ATOM/) {
...
system "/usr/bin/python $workdir/scripts/prediction_RNA.py $workdir";

########################Random forest classification#############################
system "/usr/bin/python $workdir/scripts/lf_RNA.py $workdir";
