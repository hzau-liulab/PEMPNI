...
my $tem = $pdb_name."_".$chain_name."_diff-distance";   #1AAY_A_D120A
...
open(OUT,">","$workdir/output/energy_feature/GB1/GB1_WT_EPI.txt") or die "$!"; 
my %wt_inter_pro;            
my %wt_inter_nul;            
open(INI,"<","/home/yaojiang/protein-DNA/output/distance/$pdb_chain"."_pro_nul_dis_5.txt") or die "$!";
while (<INI>) {
        chomp;
        my @tem = split(/\s+/);
        if ($tem[2] <= 5) {
...
open(OUT,">","$workdir/output/energy_feature/GB1/GB1_MT_EPI.txt") or die "$!"; 
my %mt_inter_pro;            
my %mt_inter_nul;            
open(INI,"<","$workdir/output/distance/$pdb_chain"."_pro_nul_dis_5.txt") or die "$!";
while (<INI>) {
...
open(IN,"<","$workdir/output/energy_feature/GB1/GB1_MT_EPI.txt") or die "$!";
chomp(my @mt_line=<IN>);
my @mt=split(/\s+/,$mt_line[0]);
close IN;
    
open(INN,"<","$workdir/output/energy_feature/GB1/GB1_WT_EPI.txt") or die "$!";
...
open(OUT,">","$workdir/output/energy_feature/GB1/GB1_WT_ETOR.txt") or die "$!";    
my %wt_distance;
open(INI,"<","$workdir/output/distance/$pdb_name"."_"."$chain_name"."_diff-distance.txt") or die "$!";      
while (<INI>) {
    chomp;                                                             
    my @tem = split(/\s+/);
...
open(OUT,">","$workdir/output/energy_feature/GB1/GB1_MT_ETOR.txt") or die "$!";    
my %mt_distance;
open(INI,"<","$workdir/output/distance/$pdb_name"."_"."$chain_name"."_diff-distance.txt") or die "$!";      
while (<INI>) {
    chomp;                                                             
...
open(IN,"<","$workdir/output/energy_feature/GB1/GB1_MT_ETOR.txt") or die "$!";
chomp(my @mt_line1=<IN>);
my @mt1=split(/\s+/,$mt_line1[0]);
close IN;
    
open(INN,"<","$workdir/output/energy_feature/GB1/GB1_WT_ETOR.txt") or die "$!";
...
system "/home/yaojiang/anaconda3/bin/python $workdir/scripts/prediction_DNA.py $workdir";
#
#########################Random forest classification#############################
system "/home/yaojiang/anaconda3/bin/python $workdir/scripts/lf_DNA.py $workdir";
