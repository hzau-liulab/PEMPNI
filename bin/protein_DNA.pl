...
my $workdir="/var/www/html/PEMPNI/";               #Set to your own working directory 
...
system "mkdir -p $workdir/output/energy_feature";
system "mkdir -p $workdir/output/energy_feature/GB1";
system "mkdir -p $workdir/output/distance";
system "mkdir -p $workdir/output/bio_feature";
system "mkdir -p $workdir/output/bio_feature/ASA";
system "mkdir -p $workdir/output/bio_feature/contact";
...
for my $i (@kkk){
    ...
    my $pp=substr($hesuan_lian[0],13,3);
    $pp =~s/\s+//g;
    if ($pp eq "O5'") {
     push (@flag,0);
}else{
...
while (my $line=<IN>) {
    if($line=~/^ATOM/){  
    if ($line=~/^(ATOM).*D[AGCT]/) {print OUTT "$line"; print OUTTT "$line";
                                   print OUTTTT "$line";}
...
my $tem = $pdb_name."_".$chain_name."_".$mutate_res;   #1AAY_A_D120A
my @te = split(/_/,$tem);                 #1AAY  A  D120A
my $site = $mutate_pos;                   #120              
my $wt = substr($mutate_res,0,1);         #D
my @pro_coor;                             
my %pdb_coor;                           
open(INN,"<","$workdir/output/data_process/WT_com.pdb") or die "$!";    #1AAY_A.pdb 
while (<INN>) {
    chomp;
    if ($_ =~ /^ATOM/) {
...
open(OUT,">","$workdir/output/distance/$tem.txt") or die "$!";   
...
my %res_atom;
my %res_contact;
open(IN,"<","$workdir/output/distance/$i"."_pro_nul_dis_5.txt") or die "$!";
while (<IN>) {
    chomp;
    my @tem = split(/\s+/);
    if ($tem[2] < 4.5) {                              
...
close IN;
open(OUT,">","$workdir/output/bio_feature/contact/$i"."_contact_nul.txt") or die "$!";
...
my @mt_contact_nul;
my @wt_contact_nul;
open(IN,"$workdir/output/bio_feature/contact/$pdb_chain"."_contact_nul.txt") or die "$!";
while (my $line = <IN>) {
...
open(INN,"$workdir/output/bio_feature/contact/$mut_name"."_contact_nul.txt") or die "$!";
while (my $line = <INN>) {
    my @line = split(/\s+/,$line);
    if ($line[0] == $mutate_pos ) {
...
open(OUT,">","$workdir/output/bio_feature/contact/CFNA.txt") or die "$!";
#print OUT "$wt_contact_nul[0]\t";
...
my %res_atom;
my %res_contact;
open(IN,"<","$workdir/output/distance/$i"."_pro_pro_dis_4.5.txt") or die "$!";
while (<IN>) {
    chomp;
    my @tem = split(/\s+/);
    if ($tem[2] < 4.5) {                              
...
close IN;
open(OUT,">","$workdir/output/bio_feature/contact/$i"."_contact_pro.txt") or die "$!";
foreach my $key(sort{$a <=> $b}keys%res_atom) {
    print OUT "$key\t";
    my $contact = scalar keys%{$res_contact{$key}};  
    my $density = $res_atom{$key}/$contact;          
...
my @mt_contact_pro;
my @wt_contact_pro;
open(IN,"$workdir/output/bio_feature/contact/$pdb_chain"."_contact_pro.txt") or die "$!";
while (my $line = <IN>) {
    my @line = split(/\s+/,$line);
    if ($line[0] == $mutate_pos ) {
...
open(INN,"$workdir/output/bio_feature/contact/$mut_name"."_contact_pro.txt") or die "$!";
while (my $line = <INN>) {
    my @line = split(/\s+/,$line);
    if ($line[0] == $mutate_pos ) {
...
open(OUT,">","$workdir/output/bio_feature/contact/CFAA.txt") or die "$!";
#print OUT "$wt_contact_nul[0]\t";
...
open(INN,"<","$workdir/output/bio_feature/contact/$pdb_chain"."_contact_pro.txt") or die "$!";
 while (<INN>) {
        chomp;
        $_ =~ s/^\s+//;
        my @tem = split(/\s+/);
...
close INN;
open(OUT,">","$workdir/output/bio_feature/contact/IR-CFAA.txt") or die "$!";
...
print OUT "\t";
open(INN,"$workdir/output/bio_feature/contact/CFNA.txt") or die "$!";
while (my $line = <INN>) {
    chomp $line;
    my @line = split(/\s+/,$line);
...
open(INN,"$workdir/output/bio_feature/contact/CFAA.txt") or die "$!";
while (my $line = <INN>) {
    chomp $line;
    my @line = split(/\s+/,$line);
    push(@bio_feature,@line)
...
open(INN,"$workdir/output/bio_feature/HB/NHB.txt") or die "$!";
while (my $line = <INN>) {
    chomp $line;
    my @line = split(/\s+/,$line);
     push(@bio_feature,@line)
...
open(INN,"$workdir/output/bio_feature/contact/IR-CFAA.txt") or die "$!";
while (my $line = <INN>) {
    chomp $line;
    my @line = split(/\s+/,$line);
     push(@bio_feature,@line)
...
system "mkdir -p $workdir/output/energy_feature/GB1/wt";
system "cp $workdir/output/data_process/WT_com.pdb $workdir/output/energy_feature/GB1/wt";
system "cp $workdir/output/data_process/WT_dna.pdb $workdir/output/energy_feature/GB1/wt";
system "cp $workdir/output/data_process/WT_pro.pdb $workdir/output/energy_feature/GB1/wt";
system "mv $workdir/output/energy_feature/GB1/wt/WT_com.pdb $workdir/output/energy_feature/GB1/wt/com.pdb";
system "mv $workdir/output/energy_feature/GB1/wt/WT_pro.pdb $workdir/output/energy_feature/GB1/wt/pro.pdb";
system "mv $workdir/output/energy_feature/GB1/wt/WT_dna.pdb $workdir/output/energy_feature/GB1/wt/dna.pdb";
system "cp $workdir/scripts/GB1/* $workdir/output/energy_feature/GB1/wt";
chdir "$workdir/output/energy_feature/GB1/wt";
system "sh $workdir/output/energy_feature/GB1/wt/amber.sh";
system "mv $workdir/output/energy_feature/GB1/wt/FINAL_DECOMP_MMPBSA_GB1_pair.dat $workdir/output/energy_feature/GB1/$pdb_chain'.'_pair.dat";

system "mkdir -p $workdir/output/energy_feature/GB1/mt";
system "cp $workdir/output/data_process/COM.pdb $workdir/output/energy_feature/GB1/mt";
system "cp $workdir/output/data_process/DNA.pdb $workdir/output/energy_feature/GB1/mt";
system "cp $workdir/output/data_process/MT_pro.pdb $workdir/output/energy_feature/GB1/mt";
system "mv $workdir/output/energy_feature/GB1/mt/COM.pdb $workdir/output/energy_feature/GB1/mt/com.pdb";
system "mv $workdir/output/energy_feature/GB1/mt/MT_pro.pdb $workdir/output/energy_feature/GB1/mt/pro.pdb";
system "mv $workdir/output/energy_feature/GB1/mt/DNA.pdb $workdir/output/energy_feature/GB1/mt/dna.pdb";
system "cp $workdir/scripts/GB1/* $workdir/output/energy_feature/GB1/mt";
chdir "$workdir/output/energy_feature/GB1/mt";
system "sh $workdir/output/energy_feature/GB1/mt/amber.sh";
system "mv $workdir/output/energy_feature/GB1/mt/FINAL_DECOMP_MMPBSA_GB1_pair.dat $workdir/output/energy_feature/GB1/$mut_name'.'_pair.dat";

system "cp $workdir/output/data_process/MT_com.pdb $workdir/output/energy_feature/GB1/";
system "mv $workdir/output/energy_feature/GB1/MT_com.pdb $workdir/output/energy_feature/GB1/$mut_name'.'_com.pdb";
system "cp $workdir/output/data_process/WT_com.pdb $workdir/output/energy_feature/GB1/";
system "mv $workdir/output/energy_feature/GB1/WT_com.pdb $workdir/output/energy_feature/GB1/$pdb_chain'.'_com.pdb";

###################################Process the residue pair energy decomposition file output by amber#########################
for my $i (@name){
my @data;
open(IN,"<","$workdir/output/energy_feature/GB1/$i"."._pair.dat") or die "$!";
while (<IN>) {
    chomp;
...
my %res;
my $m = 0;
open(INN,"<","$workdir/output/energy_feature/GB1/$i"."._com.pdb") or die "$!";
while (<INN>) {
    chomp;
if ($_ =~ /^ATOM/) {
...
open(OUT,">","$workdir/output/energy_feature/GB1/$i"."_pair_out.txt") or die "$!";
...
open(OUT,">","$workdir/output/energy_feature/GB1/GB1_WT_inter+intra_energy.txt") or die "$!"; 
my %wt_inter_pro;            
my %wt_inter_nul;            
open(INI,"<","$workdir/output/distance/$pdb_chain"."_pro_nul_dis_5.txt") or die "$!";
while (<INI>) {
...
open(INN,"<","$workdir/output/energy_feature/GB1/$pdb_chain"."_pair_out.txt") or die "$!";    
    while (<INN>) {
        chomp;
        $_ =~ s/^\s+//;
        my @tem = split(/\s+/);
...
open(OUT,">","$workdir/output/energy_feature/GB1/GB1_MT_inter+intra_energy.txt") or die "$!"; 
my %mt_inter_pro;            
my %mt_inter_nul;            
open(INI,"<","$workdir/output/distance/$mut_name"."_pro_nul_dis_5.txt") or die "$!";
while (<INI>) {
        chomp;
        my @tem = split(/\s+/);
        if ($tem[2] <= 5) {
...
open(INN,"<","$workdir/output/energy_feature/GB1/$mut_name"."_pair_out.txt") or die "$!";    
while (<INN>) {
        chomp;
        $_ =~ s/^\s+//;
        my @tem = split(/\s+/);
...
open(IN,"<","$workdir/output/energy_feature/GB1/GB1_MT_inter+intra_energy.txt") or die "$!";
chomp(my @mt_line=<IN>);
my @mt=split(/\s+/,$mt_line[0]);
close IN;
    
open(INN,"<","$workdir/output/energy_feature/GB1/GB1_WT_inter+intra_energy.txt") or die "$!";
chomp(my @wt_line=<INN>);
my @wt=split(/\s+/,$wt_line[0]);
close INN;
    
open(OUT,">","$workdir/output/energy_feature/GB1/EPI.txt") or die "$!";
#print OUT
foreach my $kk(0..$#mt) {
my $cha = $mt[$kk] - $wt[$kk];
print OUT $cha,"\t";
...
open(OUT,">","$workdir/output/energy_feature/GB1/GB1_WT_Ntarget_energy.txt") or die "$!";    
my %wt_distance;
open(INI,"<","$workdir/output/distance/$pdb_chain"."_"."$mutate_res.txt") or die "$!";      
while (<INI>) {
    chomp;                                                             
    my @tem = split(/\s+/);
...
while (<INN>) {
    chomp;
        ...
        if (exists $aa{$tem[3]} and $tem[5] eq $mutate_pos and $tem[2] ne $mutate_pos) {    
            next;    
        }elsif(exists $aa{$tem[0]} and $tem[2] eq $mutate_pos and $tem[5] ne $mutate_pos) {  
...
open(OUT,">","$workdir/output/energy_feature/GB1/GB1_MT_Ntarget_energy.txt") or die "$!";    
my %mt_distance;
open(INI,"<","$workdir/output/distance/$pdb_chain"."_"."$mutate_res.txt") or die "$!";      
while (<INI>) {
    chomp;                                                             
...
open(IN,"<","$workdir/output/energy_feature/GB1/GB1_MT_Ntarget_energy.txt") or die "$!";
chomp(my @mt_line1=<IN>);
my @mt1=split(/\s+/,$mt_line1[0]);
close IN;
    
open(INN,"<","$workdir/output/energy_feature/GB1/GB1_WT_Ntarget_energy.txt") or die "$!";
chomp(my @wt_line1=<INN>);
my @wt1=split(/\s+/,$wt_line1[0]);
close INN;
    
open(OUTT,">","$workdir/output/energy_feature/GB1/ETOR.txt") or die "$!";
#print OUT
foreach my $kk(0..$#mt1) {
...
system "/usr/bin/python $workdir/scripts/prediction_DNA.py $workdir";

########################Random forest classification#############################
system "/usr/bin/python $workdir/scripts/lf_DNA.py $workdir";
