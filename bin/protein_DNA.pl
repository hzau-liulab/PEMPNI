...
for my $k (@name){
my @data;
open(IN,"<","$workdir/output/energy_feature/GB1/$k"."._pair.dat") or die "$!";
while (<IN>) {
    chomp;
...
my %res;
my $m = 0;
open(INN,"<","$workdir/output/energy_feature/GB1/$k"."._com.pdb") or die "$!";
while (<INN>) {
    chomp;
if ($_ =~ /^ATOM/) {
...
open(OUT,">","$workdir/output/energy_feature/GB1/$k"."_pair_out.txt") or die "$!";
